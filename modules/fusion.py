...
class FeatureFusion(nn.Module):
    def __init__(self, convolutional_layer_encoding="z", mask_type="basic", add_origin_feat_flag=True):
        ...
        self.rgb_decoder_layer4 = deconvbnrelu(in_channels=256, out_channels=128, kernel_size=5, stride=2, padding=2,
                                               output_padding=1)
        self.rgb_decoder_layer2 = deconvbnrelu(in_channels=128, out_channels=32, kernel_size=5, stride=2, padding=2,
                                               output_padding=1)
        self.rgb_decoder_layer0 = deconvbnrelu(in_channels=32, out_channels=16, kernel_size=3, stride=1, padding=1,
                                               output_padding=0)
        self.rgb_decoder_layer = deconvbnrelu(in_channels=16, out_channels=8, kernel_size=5, stride=2, padding=2,
                                              output_padding=1)
        self.rgb_decoder_output = deconvbnrelu(in_channels=8, out_channels=2, kernel_size=3, stride=1, padding=1,
                                               output_padding=0)
        ...
        self.decoder_layer3 = deconvbnrelu(in_channels=256, out_channels=128, kernel_size=5, stride=2, padding=2,
                                           output_padding=1)
        self.decoder_layer4 = deconvbnrelu(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1,
                                           output_padding=0)
        self.decoder_layer5 = deconvbnrelu(in_channels=64, out_channels=32, kernel_size=5, stride=2, padding=2,
                                           output_padding=1)
        self.decoder_layer6 = deconvbnrelu(in_channels=32, out_channels=16, kernel_size=3, stride=1, padding=1,
                                           output_padding=0)
        self.decoder_layer7 = deconvbnrelu(in_channels=16, out_channels=8, kernel_size=5, stride=2, padding=2,
                                           output_padding=1)

        # output
        self.rgbdepth_decoder_stage1 = deconvbnrelu(in_channels=32, out_channels=16, kernel_size=5, stride=2, padding=2,
                                                    output_padding=1)
        self.rgbdepth_decoder_stage2 = deconvbnrelu(in_channels=16, out_channels=8, kernel_size=5, stride=2, padding=2,
                                                    output_padding=1)
        self.rgbdepth_decoder_stage3 = deconvbnrelu(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1,
                                                    output_padding=0)

        self.final_decoder_stage1 = deconvbnrelu(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1,
                                                 output_padding=0)
        self.final_decoder_stage2 = deconvbnrelu(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1,
                                                 output_padding=0)
        self.final_decoder_stage3 = deconvbnrelu(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1,
                                                 output_padding=0)
    ...
    def forward(self, rgb, depth, confidence, depth_values, stage_idx, origin_feat, intrinsics_matrices_stage):
        ...
        # depth_min, depth_max = depth_values[:, 0, None, None], depth_values[:, -1, None, None]
        ...
        if self.mask_type == "basic":
            ...
            valid_mask = torch.where(torch.logical_and(d > 0, confidence > confidence.mean()), torch.full_like(d, 1.0),
...
class GeoRegNet2d(nn.Module):
    def __init__(self, input_channel=128, base_channel=32, convolutional_layer_encoding="std"):
        ...
        self.encoder_layer1 = Reg_BasicBlockGeo(inplanes=8, planes=16, kernel_size=(1, 3, 3), stride=(1, 2, 2),
                                                padding=(0, 1, 1), geoplanes=self.geoplanes)
        self.encoder_layer2 = Reg_BasicBlockGeo(inplanes=16, planes=32, kernel_size=(1, 3, 3), stride=1,
                                                padding=(0, 1, 1), geoplanes=self.geoplanes)
        self.encoder_layer3 = Reg_BasicBlockGeo(inplanes=32, planes=64, kernel_size=(1, 3, 3), stride=(1, 2, 2),
                                                padding=(0, 1, 1), geoplanes=self.geoplanes)
        self.encoder_layer4 = Reg_BasicBlockGeo(inplanes=64, planes=128, kernel_size=(1, 3, 3), stride=1,
                                                padding=(0, 1, 1), geoplanes=self.geoplanes)
        self.encoder_layer5 = Reg_BasicBlockGeo(inplanes=128, planes=256, kernel_size=(1, 3, 3), stride=(1, 2, 2),
                                                padding=(0, 1, 1), geoplanes=self.geoplanes)

        self.decoder_layer4 = reg_deconvbnrelu(in_channels=256, out_channels=128, kernel_size=(1, 5, 5),
                                               stride=(1, 2, 2), padding=(0, 2, 2), output_padding=(0, 1, 1))
        self.decoder_layer3 = reg_deconvbnrelu(in_channels=128, out_channels=64, kernel_size=(1, 3, 3), stride=1,
                                               padding=(0, 1, 1), output_padding=0)
        self.decoder_layer2 = reg_deconvbnrelu(in_channels=64, out_channels=32, kernel_size=(1, 5, 5), stride=(1, 2, 2),
                                               padding=(0, 2, 2), output_padding=(0, 1, 1))
        self.decoder_layer1 = reg_deconvbnrelu(in_channels=32, out_channels=16, kernel_size=(1, 3, 3), stride=1,
                                               padding=(0, 1, 1), output_padding=0)
        self.decoder_layer = reg_deconvbnrelu(in_channels=16, out_channels=8, kernel_size=(1, 5, 5), stride=(1, 2, 2),
                                              padding=(0, 2, 2), output_padding=(0, 1, 1))

        self.prob = reg_deconvbnrelu(in_channels=8, out_channels=1, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1),
                                     output_padding=0)
...
def regconv1x1(in_planes, out_planes, kernel_size, stride, padding, groups=1, bias=False):
    return nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, groups=groups,
                     bias=bias)
