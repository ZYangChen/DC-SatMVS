...
class FeatureNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.arch_mode == "unet":
            ...
            if self.num_stage == 3:
                # print('intra_feat1',intra_feat.shape, conv1.shape)
                intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner1(conv1)
                out = self.out2(intra_feat)
                # print('intra_feat2',intra_feat.shape, out.shape)
                outputs["stage2"] = out

                intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner2(conv0)
                # print('conv0',conv0.shape)
                out = self.out3(intra_feat)
                # print('intra_feat3',intra_feat.shape, out.shape)
                outputs["stage3"] = out
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # numpy_array = attn.cpu().detach().numpy()
        # np.save("./attn.npy", numpy_array)
...
class TransformerNet(nn.Module):
    def __init__(self,
        ...
        print("*************feature supply arch mode:{}****************".format("Transformer"))
        self.arch_mode = arch_mode
        self.stride = stride
        self.base_channels = base_channels
        self.num_stage = num_stage
        ...
        '''
        self.pe = Conv2d(base_channels * 2 ** 2, base_channels * 2, 3, 1, padding=1)
        self.tr1 = nn.Sequential(*[
            TransformerBlock(dim=base_channels * 2, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
        ...
        '''
        self.dw1_2 = nn.Conv2d(base_channels * 2 * 4, base_channels * 2 ** 2, kernel_size=3, stride=1, padding=1, bias=False)
        self.tr2 = nn.Sequential(*[
            TransformerBlock(dim=base_channels * 2 ** 2, num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        # self.out1 = nn.Conv2d(base_channels * 4, base_channels * 4, 1, bias=False)
        self.out1 = nn.Sequential(*[
            TransformerBlock(dim=base_channels * 2 ** 2, num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
    ...
    def forward(self, x):
        conv0 = self.conv0(x)
        # print('conv0',conv0.shape)
        pe = pixel_unshuffle(conv0, 2)
        # conv1 = self.conv1(conv0)
        # print('pe',pe.shape)
        conv1 = self.pe(pe)
        # print('conv1pe',conv1.shape)
        del pe
        conv1 = self.tr1(conv1)
        # print('conv1',conv1.shape)
        # conv2 = self.conv2(conv1)
        conv2 = pixel_unshuffle(conv1, 2)
        # print('conv21',conv2.shape)
        conv2 = self.dw1_2(conv2)
        conv2 = self.tr2(conv2)
        # print('conv22',conv2.shape)
