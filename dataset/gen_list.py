...
def gen_ref_list_cam_whu(data_folder, files_name, view_num, ref_view=2):
    sample_list = []

    # image_folder = os.path.join(data_folder, ('Images/%s/%s' %files_name % ref_view)).replace("\\", "/")
    # image_folder = os.path.join(data_folder, ('Images', '%s', '%s' % (files_name, ref_view))).replace("\\", "/")
    image_folder = os.path.join(data_folder, 'Images', str(files_name), str(ref_view)).replace("\\", "/")
    # print('image',image_folder)
    # rpc_folder = os.path.join(data_folder, ('Cams/*/%s' % ref_view)).replace("\\", "/")
    rpc_folder = os.path.join(data_folder, 'Cams', str(files_name), str(ref_view)).replace("\\", "/")
    # height_folder = os.path.join(data_folder, ('Depths/*/%s' % ref_view)).replace("\\", "/")
    height_folder = os.path.join(data_folder, 'Depths', str(files_name), str(ref_view)).replace("\\", "/")
    ...
    for p in image_files:
        ...
        ref_image = os.path.join(image_folder, '{}.png'.format(name)).replace("\\", "/")
        ref_rpc = os.path.join(rpc_folder, '{}.txt'.format(name)).replace("\\", "/")
        ref_height = os.path.join(height_folder, '{}.png'.format(name)).replace("\\", "/")
        ...
        for s in range(view_num):
            ...
            if sv != ref_view:
                # source_image = os.path.join(data_folder, 'Images/*/{}/{}.png'.format(sv, name)).replace("\\", "/")
                source_image = os.path.join(data_folder, 'Images/{}/{}/{}.png'.format(files_name, sv, name)).replace("\\", "/")
                # source_rpc = os.path.join(data_folder, 'Cams/*/{}/{}.txt'.format(sv, name)).replace("\\", "/")
                source_rpc = os.path.join(data_folder, 'Cams/{}/{}/{}.txt'.format(files_name, sv, name)).replace("\\", "/")
